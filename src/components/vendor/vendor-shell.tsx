...
const navigation = [
  { label: "Home", to: "/vendor", icon: IconHome },
  { label: "My Loads", to: "/vendor/loads", icon: IconTruckDelivery },
  { label: "Payments", to: "/vendor/payments", icon: IconCash },
  { label: "Documents", to: "/vendor/documents", icon: IconFiles },
  { label: "Profile", to: "/vendor/profile", icon: IconUser },
] as const
...
export function VendorShell({
  ...
  const active = (to: string) =>
    to === "/vendor" ? pathname === to : pathname.startsWith(to)
  ...
  return (
    <div className="min-h-svh bg-muted/30">
      ...
      <div className="lg:pl-64">
        ...
        <main className="mx-auto w-full max-w-6xl p-4 pb-24 sm:p-6 lg:p-8 lg:pb-8">
          <Outlet />
        </main>
      </div>
      <nav
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.to}
                className={cn(
                  "flex min-h-12 flex-col items-center justify-center gap-1 px-1 text-center text-[0.6875rem] text-muted-foreground",
